...
import numpy as np 
from torch.distributions import MultivariateNormal  # 多变量正态分布
...
################# PPO algorithm ##############
"""
expereince replay buffer : rolloutbuffer
定义了初始化和清空两种操作
"""
class RolloutBuffer:
    def __init__(self):
...
class ActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim, has_continuous_action_space, action_std_init):
        ...
        if has_continuous_action_space: # 判断是否为连续动作空间，如果是离散的则不需要这一步
            self.action_dim = action_dim
            self.action_var = T.full((action_dim,), action_std_init * action_std_init).to(device)
    ...
    def set_action_std(self, new_action_std):
        # 对连续动作空间设置动作标准差，离散空间则不需要考虑
        if self.has_continuous_action_space:
            self.action_var = T.full((self.action_dim,), new_action_std * new_action_std).to(device)
        else:
            print("--------------------------------------------------------------------------------------------")
    ...
    def evaluate(self, state, action):
        ...
        if self.has_continuous_action_space:
            ...
            cov_mat = T.diag_embed(action_var).to(device)
            dist = MultivariateNormal(action_mean, cov_mat)
