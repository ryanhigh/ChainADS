...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, o_data):
    ...
    def forward(self, state, isTrain=True):
        if isTrain:
            ...
            z1_new = self.p * z1_actor_detached + (1-self.p)*(0.3 * o_data1_0_detached+0.7*o_data2_0_detached)
            ...
            z2_new = self.p * z2_actor_detached + (1-self.p)*(0.3 * o_data1_1_detached +0.7*o_data2_1_detached)
            z2_new = z2_new.requires_grad_()

            z2_actor = T.tanh(z2_new)
            z3_actor = self.actor_fc3(z2_actor)

            z3_actor_detached = z3_actor.detach().to(device)  # 假设 z3_actor 是一个 PyTorch 张量
            o_data1_2_detached = T.tensor(self.o_data1[2], dtype=T.float32).detach().to(device)  # 确保是张量
            o_data2_2_detached = T.tensor(self.o_data2[2], dtype=T.float32).detach().to(device)  # 确保是张量

            z3_new = self.p * z3_actor_detached + (1-self.p)*(0.3 * o_data1_2_detached+0.7*o_data2_2_detached)
            z3_new = z3_new.requires_grad_()
            z3_actor = T.softmax(z3_new, dim=-1)
        else:
            z1_actor = self.actor_fc1(state)
...
class Critic(nn.Module):
    ...
    def forward(self, state, isTrain=True):
        if isTrain:
            ...
            z1_new = self.p * z1_critic_detached + (1-self.p)*(0.3 * o_data1_0_detached+0.7*o_data2_0_detached)
            z1_new = z1_new.requires_grad_()
            ...
            z2_new = self.p * z2_critic_detached + (1-self.p)*(0.3 * o_data1_1_detached+0.7*o_data2_1_detached)
            z2_new = z2_new.requires_grad_()

            z2_critic = T.tanh(z2_new)
            z3_critic = self.critic_fc3(z2_critic)

            z3_critic_detached = z3_critic.detach().to(device)  # 假设 z1_actor 是一个 PyTorch 张量
            o_data1_2_detached = T.tensor(self.o_data1[2], dtype=T.float32).detach().to(device)  # 确保是张量
            o_data2_2_detached = T.tensor(self.o_data2[2], dtype=T.float32).detach().to(device)  # 确保是张量

            z3_critic = self.p * z3_critic_detached + (1-self.p)*(0.3 * o_data1_2_detached+0.7*o_data2_2_detached)
            z3_critic = z3_critic.requires_grad_()
        else:
            z1_critic = self.critic_fc1(state)
            z1_critic = T.tanh(z1_critic)
            z2_critic = self.critic_fc2(z1_critic)
            z2_critic = T.tanh(z2_critic)
            z3_critic = self.critic_fc3(z2_critic)
        return z3_critic
...
class ActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        #                 nn.Linear(64, 1)
        #             )
        # reshape模块，用于修改对应层的输出
        self.o_data = None
        
        self.actor = Actor(state_dim, action_dim, self.o_data)
        self.critic = Critic(state_dim, action_dim, self.o_data)
    ...
    def act(self, state, isTrain):
        ...
        state_val = self.critic(state, isTrain)
...
class PPO:
    ...
    def choose_action(self, state, isTrain=True):
        with T.no_grad():
            ...
            action, action_logprob, state_val = self.policy_old.act(state, isTrain)
