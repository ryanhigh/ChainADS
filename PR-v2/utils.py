...
def LabberRing(windowsize, episodes, baseline, ddqn, ppo, title, ylabel, figure_file):
    ...
    plt.figure()
    plt.plot(episodes, ppo, linestyle='-', linewidth = 2, color='lightcoral', label='pr', alpha=0.2)
    plt.plot(episodes, ppo_av, linestyle=':', linewidth = 2, color='r', label='pr-smooth')
    plt.plot(episodes, ddqn, linestyle='-', linewidth = 2, color='skyblue', label='nopr', alpha=0.2)
    plt.plot(episodes, ddqn_av, linestyle='--', linewidth = 2, color='b', label='nopr-smooth')
    plt.plot(episodes, baseline, linestyle='-', linewidth = 2, color='c', label='baseline')
    plt.plot(episodes, ddqn_mean, linestyle='--', linewidth = 2, color='k')
    plt.plot(episodes, ppo_mean, linestyle=':', linewidth = 2, color='k')
    # plt.text(4900, np.mean(ddqn)-1, "noPR")
    # plt.text(5000, np.mean(ppo)+1, "PR")
