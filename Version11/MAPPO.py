...
class MAPPO:
    def __init__(self, system_state_dim, node_state_dim, action_dim, num_agents, lr_actor, lr_critic, gamma, K_epochs, eps_clip):
        self.epsilon = 1.0
        self.eps_min = 0.01
        self.eps_dec = 5e-4
        ...
        self.policies = [ActorCritic(system_state_dim, node_state_dim, action_dim).to(device) for _ in range(self.num_agents)]
        self.optimizers = [T.optim.Adam([
                        {'params': self.policy.actor.parameters(), 'lr': lr_actor},
                        {'params': self.policy.critic.parameters(), 'lr': lr_critic}
                    ]) for i in range(self.num_agents)]

        self.policy_olds = [ActorCritic(system_state_dim, node_state_dim, action_dim).to(self.device) for _ in range(self.num_agents)]
        for i in range(self.num_agents):
    ...
    def choose_action(self, state):
        with T.no_grad():
            ...
            for i in range(4):
                ...
                action, action_logprob = self.policy_old[i].act(state)  
    ...
    def update(self):
       ...
       # convert list to tensor
        old_states = T.squeeze(T.stack(self.buffers[j].states, dim=0)).detach().to(device)
        old_actions = T.squeeze(T.stack(self.buffers[j].actions, dim=0)).detach().to(device)
        old_logprobs = T.squeeze(T.stack(self.buffers[j].logprobs, dim=0)).detach().to(device)
        old_state_values = T.squeeze(T.stack(self.buffers[j].state_values, dim=0)).detach().to(device)
