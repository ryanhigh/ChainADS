...
def policy_selection(W, global_steps):
    temperature = global_steps / Max_training_timesteps
    probs = np.exp(temperature * W) / np.sum(np.exp(temperature * W))
    selected_index = np.random.choice(len(W), p=probs)
    return selected_index, probs, temperature
...
def ppo_train():
    ...
    # temperature = 1.0   # 温度因子
    # delta_tau = -0.00001     # 改变值
    psi = 0.7
    ...
    while time_step <= max_training_timesteps:
        # select policy
        selected_ind, probs, temperature = policy_selection(W, time_step)
        ...
        for t in range(1, max_ep_len+1):
            ...
            if selected_ind == 0:
                ...
                if np.random.rand() < psi:
                    ...
                    # epsilon greedy pol
                    action = agent.epsilon_choose_action(observation)
        ...
        # gain_norm = 2 / (2 + np.exp(-avg_reward))
        gain_norm = avg_reward
        W[selected_ind] = (W[selected_ind] * U[selected_ind] + gain_norm) / (U[selected_ind] + 1)
        U[selected_ind] += 1
