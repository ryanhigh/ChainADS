...
class RecursiveLayer(nn.Module):
    ...
    def forward(self, x1, x2, x3, x4):
        ...
        h2 = h1 + self.W(x2) + self.b
        h2 = h2 * torch.sigmoid(h2)
        
        h3 = h2 + self.W(x3) + self.b
        h3 = h3 * torch.sigmoid(h3)
        
        h4 = h3 + self.W(x4) + self.b
        h4 = h4 * torch.sigmoid(h4)
...
class ActorCritic(nn.Module):
    def __init__(self, system_state_dim, node_state_dim, action_dim, hidden_dim=64, num_layers=1):
        ...
        self.recursive_layers = nn.ModuleList([RecursiveLayer(node_state_dim, 16) for _ in range(num_layers)])
    ...
    def act(self, system_state, node_states):
        ...
        for i in range(self.num_layers):
            h1, h2, h3, h4 = self.recursive_layers[i](node_states[0], node_states[1], node_states[2], node_states[3])
